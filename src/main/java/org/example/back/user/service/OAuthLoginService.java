...
public class OAuthLoginService {
	...
	public SignInResponseDto login(OAuthLoginParams params) {
		...
		if (userRepository.findByEmail(oAuthInfoResponse.getEmail()).isPresent()){
			userEntity= userRepository.findByEmail(oAuthInfoResponse.getEmail());
		}
		else {
			userEntity= Optional.ofNullable(findOrCreateMember(oAuthInfoResponse));
		}

		Optional<Profile> profile = profileRepository.findByUserId(userEntity.get().getUserId());
		...
		return new SignInResponseDto(authTokensGenerator.generate(userEntity.get().getUserId()),3600L, hasProfile,hasProfileImage,hasProfileLocation);
	}
